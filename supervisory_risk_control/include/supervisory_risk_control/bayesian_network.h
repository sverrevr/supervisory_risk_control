...
private:
    ...
    std::deque<map_stringkey<map_stringkey<double>>> temporal_leaf_node_states_;

    auto getNodeId(std::string name) const
    {
        const auto node_id = net_.FindNode(name.c_str());
        if (node_id < 0)
            ROS_ERROR("Node name \"%s\" resulted in error", name.c_str());
        assert(node_id >= 0);
        return node_id;
    }
    ...
    auto getOutcomeId(std::string node_name, std::string outcome_name)
    {
        ...
        if (outcome_id < 0)
            ROS_ERROR("Outcome name \"%s\" resulted in error for node name \"%s\"", outcome_name.c_str(), node_name.c_str());
        assert(outcome_id >= 0);
        return outcome_id;
    }
    ...
    void setDefinition(std::string node_name, DSL_doubleArray &CPT)
    {
        ...
        if (sum < 0.9999 || sum > 1.00001 || !isfinite(sum))
            ROS_ERROR("Prior distribution on \"%s\" sums to %f, should be 1", node_name.c_str(), sum);
        ...
        //}
        if (result < 0)
            ROS_ERROR("Setting priors failed on node \"%s\"", node_name.c_str());
        assert(result >= 0);
    }
    ...
    void wrapTimeSlice(int *time_slice)
    {
        ...
        if (*time_slice >= num_slices)
            ROS_ERROR("Attempted to access nonexisting timeslice %d", *time_slice);
    ...
    {
        map_stringkey<double> return_value;
        const auto node_id = getNodeId(node_name);
    ...
    void apply_evidence()
    {
        ...
        for (const auto &[node_id, outcome_id] : evidence_)
        {
            ...
            if (res < 0)
                ROS_ERROR("Set evidence (outcome_id=%d) on node \"%d\" resulted in error", outcome_id, node_id);
            assert(res >= 0);
        }
        for (const auto &[node_id, evidence_vec] : virtual_evidence_)
        {
            ...
            if (res < 0)
                ROS_ERROR("Set virtual evidence on node \"%d\" resulted in error", node_id);
            assert(res >= 0);
        }
        int i = 0;
        for (auto evidence : temporal_evidence_)
        {
            for (const auto &[node_id, outcome_id] : evidence)
            {
                ...
                if (res < 0)
                    ROS_ERROR("Set temporal evidence (slice=%d, outcome_id=%d) on node \"%d\" resulted in error", i, outcome_id, node_id);
                assert(res >= 0);
            }
            ++i;
        }
        ...
        for (auto virtual_evidence : temporal_virtual_evidence_)
        {
            for (const auto &[node_id, evidence_vec] : virtual_evidence)
            {
                ...
                if (res < 0)
                    ROS_ERROR("Set temporal evidence (slice=%d) on node \"%d\" resulted in error", i, node_id);
                assert(res >= 0);
            }
            ++i;
        }
...
public:
        ...
        : max_number_of_timesteps_(max_num_timesteps), temporal_leaf_nodes_(temporal_leaf_nodes)
    {
        ...
        if (result < 0)
            ROS_ERROR("Unable to read file: \"%s\"", full_path.c_str());
        assert(result >= 0);
        const auto result_slice = net_.SetNumberOfSlices(1);
        if (result_slice < 0)
            ROS_ERROR("Insufficient number of time slices");
        ...
        else
        {
            ...
            if (return_set_samples < 0)
                ROS_ERROR("Illigal number of samples set: %d", num_network_evaluation_samples);
    ...
    void save_network(std::string full_path)
    {
        ...
        if (result < 0)
            ROS_ERROR("Unable to write file: \"%s\"", full_path.c_str());
        assert(result >= 0);
    ...
    void setEvidence(std::string node_name, int outcome_id, int time_slice = -1)
    {
        ...
        else
        {
            ...
            const auto res = net_.GetNode(node_id)->Value()->SetEvidence(outcome_id);
            if (res < 0)
                ROS_ERROR("Set evidence (outcome_id=%d) on node \"%s\" resulted in error", outcome_id, node_name.c_str());
            assert(res >= 0);
        }
    }
    ...
    void setEvidence(const map_stringkey<std::string> &evidence)
    {
        for (auto e : evidence)
        {
    ...
    void setEvidence(const map_stringkey<int> &evidence)
    {
        for (auto e : evidence)
        {
            setEvidence(e.first, e.second);
    ...
    {
        setPriors(node_name, std::map<std::string, double>{{"false", 1 - probablity_of_true}, {"true", probablity_of_true}});
    }
    ...
    void setVirtualEvidence(std::string node_name, const std::vector<double> &virtualEvidence, int time_slice = -1)
    {
        ...
        if (isTemporal(node_id))
        {
            ...
            if (result < 0)
                ROS_ERROR("Set virtual evidence (t=%d) on node \"%s\" resulted in error", time_slice, node_name.c_str());
            assert(result >= 0);
        }
        else
        {
            ...
            if (result < 0)
                ROS_ERROR("Set virtual evidence on node \"%s\" resulted in error", node_name.c_str());
            assert(result >= 0);
        }
    }

    void setVirtualEvidence(std::string node_name, const map_stringkey<double> &virtualEvidence)
    {
    ...
    auto evaluateStates(const std::vector<std::string> &node_names)
    {
        ...
        map_stringkey<map_stringkey<double>> return_value;
        for (auto node_name : node_names)
        {
    ...
    void incrementTime()
    {
        ...
        if (max_number_of_timesteps_ < 0 || number_of_time_slices < max_number_of_timesteps_)
        {
            ...
            if (res < 0)
                ROS_ERROR("Increment time failed");
            assert(res >= 0);
        }
        else
        {
